            ...
            if current_key_dump is None:
                ...
                for j in current_key_dump:
                    with open('%s/%s.json' % (path,j[0]),'w+') as f:
                        f.write(j[1])
                    j_data = loads(j[1])
                    email.append(j_data['client_email'] + ',')

                with open('client_emails.txt', 'w+') as f:
                    f.writelines(email)
...
if __name__ == '__main__':
    ...
    parse.add_argument('--list-sas',metavar='PROJRCT_ID',default=False,help='List service accounts in a project.')
    parse.add_argument('--create-projects',metavar='PROJRCT_ID',type=int,default=None,help='Creates up to N projects.')
    parse.add_argument('--max-projects',type=int,default=12,help='Max amount of project allowed. Default: 12')
    parse.add_argument('--enable-services',metavar='PROJRCT_ID',default=None,help='Enables services on the project. Default: IAM and Drive')
    ...
    parse.add_argument('--create-sas',metavar='PROJRCT_ID',default=None,help='Create service accounts in a project.')
    parse.add_argument('--delete-sas',metavar='PROJRCT_ID',default=None,help='Delete service accounts in a project.')
    parse.add_argument('--download-keys',metavar='PROJRCT_ID',default=None,help='Download keys for all the service accounts in a project.')
